...
class SeesawLossWithLogits(nn.Module):
    ...
    def __init__(self, class_counts = None, df: pd.DataFrame = None, p: float = 0.8):
        super().__init__()

        if class_counts is None and df is not None:
            class_counts = df[TARGET_KEY_NAME].value_counts().sort_index().values
        elif class_counts is None and df is None:
            raise ValueError
        ...
        self.eps = 1.0e-6
        print(f"Seesaw Loss With Logits with: {self.num_labels} classes initialized")
